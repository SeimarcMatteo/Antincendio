...
class FormPianificazioneIntervento extends Component
{
    ...
    public function mount()
    {
        ...
        $this->zoneDisponibili = Cliente::whereHas('presidi')
        ->whereJsonContains('mesi_visita', str_pad(now()->month, 2, '0', STR_PAD_LEFT))
        ->pluck('zona')
        ->filter()
        ->unique()
        ->sort()
        ...
        ->toArray();
            ...
            // ✅ Avvio automatico generazione interventi
       // $this->generaInterventiMancanti();
    }
    ...
    {
        return \App\Models\Intervento::where('cliente_id', $clienteId)
            ...
            $this->zoneDisponibili = Cliente::whereHas('presidi')
    ->whereJsonContains('mesi_visita', str_pad($this->meseSelezionato, 2, '0', STR_PAD_LEFT))
    ->pluck('zona')
    ->filter()
    ->unique()
    ->sort()
    ...
    public function getClientiInScadenzaProperty(): Collection
    {
        ...
        return Cliente::with(['sedi.presidi', 'presidi'])
            ...
            ->where(function ($q) use ($mese) {
                ...
                $meseStr = json_encode(str_pad($mese, 2, '0', STR_PAD_LEFT)); // "04"
            ...
            ->filter(function ($cliente) {
                // Se ha sedi, almeno una sede NON deve essere già pianificata
    ...
    public function getClientiConInterventiEsistentiProperty(): Collection
    {
        ...
        return Cliente::with(['sedi.presidi', 'presidi'])
            ...
            ->whereHas('presidi')
            ->where(function ($q) use ($mese) {
                $meseInt = json_encode((int) $mese);   // es. "4"
                $meseStr = json_encode($mese);         // es. "04"
                $q->whereRaw("JSON_CONTAINS(mesi_visita, ?)", [$meseInt])
                  ->orWhereRaw("JSON_CONTAINS(mesi_visita, ?)", [$meseStr]);
            })
            ->get()
            ->filter(function ($cliente) {
                return $cliente->sedi->contains(fn($sede) =>
                            $this->interventoEvasa($cliente->id, $sede->id) || $this->interventoEsistente($cliente->id, $sede->id)
                        )
                    || (
                        $cliente->presidi->whereNull('sede_id')->isNotEmpty() &&
                        ($this->interventoEvasa($cliente->id, null) || $this->interventoEsistente($cliente->id, null))
                    );
            });
    }
